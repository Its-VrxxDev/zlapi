...
class ZaloAPI(object):
	...
	async def login(self, phone=None, password=None, imei=None, user_agent=None):
		...
		Raises:
			...
			LoginMethodNotSupport: If method login not support
		"""
		await self.onLoggingIn("using Cookies") if await self.getSession() else await self.onLoggingIn(phone)
		...
		try:
			...
			self.user_id = (await self.fetchAccountInfo()).profile.get("userId")
	...
	async def replyTo(self, replyMsg, message, thread_id, thread_type, ttl=0):
	...
	async def sendRemoteImage(self, imageUrl, thread_id, thread_type, width=2560, height=2560, message=None, ttl=0, custom_payload=None):
	...
	async def sendReport(self, user_id, reason=0, content=None):
		"""Send report to Zalo.
	...
	async def _listen(self, thread=False, reconnect=5):
		...
		def on_error(ws, error):
			asyncio.run(self.onErrorCallBack(error))
		...
		try:
			...
			if version == 1 and cmd == 3000 and subCmd == 0:
				...
				for message in userMsgs:
					...
					[
						self.run_in_thread(self.onMessage, msgObj.msgId, str(int(msgObj.uidFrom) or self.user_id), msgObj.content, msgObj, str(int(msgObj.uidFrom) or msgObj.idTo), ThreadType.USER)
						if self.thread else
						await self.onMessage(msgObj.msgId, str(int(msgObj.uidFrom) or self.user_id), msgObj.content, msgObj, str(int(msgObj.uidFrom) or msgObj.idTo), ThreadType.USER)
					]
				...
				for message in groupMsgs:
					...
					msgObj = MessageObject.fromDict(message, None)
					[
						self.run_in_thread(self.onMessage, msgObj.msgId, str(int(msgObj.uidFrom) or self.user_id), msgObj.content, msgObj, str(int(msgObj.idTo) or self.user_id), ThreadType.GROUP)
						if self.thread else
						await self.onMessage(msgObj.msgId, str(int(msgObj.uidFrom) or self.user_id), msgObj.content, msgObj, str(int(msgObj.idTo) or self.user_id), ThreadType.GROUP)
					]
				...
				for control in controls:
					if control["content"]["act_type"] == "group":
						...
						[
							self.run_in_thread(self.onEvent, event_data, event_type)
							if self.thread else
