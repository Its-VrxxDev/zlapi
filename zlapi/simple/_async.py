			...
			if version == 1 and cmd == 3000 and subCmd == 0:
				...
				for message in user_msgs:
					...
					context = ContextObject.fromDict({
						...
						"author_id": str(int(msg_obj.uidFrom) or self.uid),
						"message": msg_obj.content,
						"message_object": msg_obj,
						"thread_id": str(int(msg_obj.uidFrom) or self.uid),
				...
				for message in group_msgs:
					...
					context = ContextObject.fromDict({
						...
						"message": msg_obj.content,
						"message_object": msg_obj,
						"thread_id": str(int(msg_obj.idTo) or self.uid),
				...
				for react in reacts:
					...
					context = {"message_id": msgObj.msgId, "author_id": str(int(msgObj.uidFrom) or self.uid), "message": msgObj.content, "message_object": msgObj, "thread_id": str(int(msgObj.uidFrom) or self.uid), "thread_type": ThreadType.USER}
				...
				for reactGroup in reactGroups:
					...
					context = {"message_id": msgObj.msgId, "author_id": int(msgObj.uidFrom) or self.uid, "message": msgObj.content, "message_object": msgObj, "thread_id": int(msgObj.idTo) or self.uid, "thread_type": ThreadType.GROUP}
					context = ContextObject.fromDict(context)
					[
						self.run_in_thread(self.onMessage, context)
						if self.thread else
